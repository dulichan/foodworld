<?php
/*
  $Id: categories_s_products.php,v 1.146 2003/07/11 14:40:27 hpdl Exp $
      ...
      case 'update_product':
        if (isset($HTTP_POST_VARS['edit_x']) || isset($HTTP_POST_VARS['edit_y'])) {
          $action = 'new_product';
            ...
            $suppliers_products_query = tep_db_query("select p.suppliers_id, s.suppliers_id, s.suppliers_percentage from " . TABLE_PRODUCTS . " p, " . TABLE_SUPPLIERS . " s where p.suppliers_id = s.suppliers_id and p.suppliers_id = '" . $suppliers_id . "'");
          ...
          $sql_data_array = array('products_quantity' => tep_db_prepare_input($HTTP_POST_VARS['products_quantity']),
                                  ...
                                  'products_price' => tep_db_prepare_input($HTTP_POST_VARS['products_price']+($HTTP_POST_VARS['products_price']*$suppliers_percentage)),
								  'products_price_myself' => tep_db_prepare_input($HTTP_POST_VARS['products_price']),
          ...
          if ($action == 'insert_product') {
            ...
            $insert_sql_data = array('products_date_added' => 'now()', 'suppliers_id' => $suppliers_id);
      ...
      case 'copy_to_confirm':
        if (isset($HTTP_POST_VARS['products_id']) && isset($HTTP_POST_VARS['categories_id'])) {
          ...
          if ($HTTP_POST_VARS['copy_as'] == 'link') {
            ...
            tep_db_query("insert into " . TABLE_PRODUCTS . " (products_quantity, products_model,products_image, products_price, products_date_added, products_date_available, products_weight, products_status, products_tax_class_id, manufacturers_id, suppliers_id) values ('" . tep_db_input($product['products_quantity']) . "', '" . tep_db_input($product['products_model']) . "', '" . tep_db_input($product['products_image']) . "', '" . tep_db_input($product['products_price']) . "',  now(), " . (empty($product['products_date_available']) ? "null" : "'" . tep_db_input($product['products_date_available']) . "'") . "', '" . tep_db_input($product['products_weight']) . "', '0', '" . (int)$product['products_tax_class_id'] . "', '" . (int)$product['manufacturers_id'] . "', '" . (int)$suppliers_id . "')");
...
<?php
  if ($action == 'new_product') {
    $parameters = array('products_name' => '',
                       ...
                       'products_status' => '',
                       'products_tax_class_id' => '',
                       'products_price_myself' => '',
                       'suppliers_id' => '');

    $pInfo = new objectInfo($parameters);

    if (isset($HTTP_GET_VARS['pID']) && empty($HTTP_POST_VARS)) {
      $product_query = tep_db_query("select pd.products_name, pd.products_description,  pd.products_url, p.products_id, p.products_quantity, p.products_model, p.products_image, p.products_price, p.products_weight, p.products_date_added, p.products_last_modified, date_format(p.products_date_available, '%Y-%m-%d') as products_date_available, p.products_status, p.products_tax_class_id, p.manufacturers_id, p.products_price_myself, p.suppliers_id from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd where p.products_id = '" . (int)$HTTP_GET_VARS['pID'] . "' and p.products_id = pd.products_id and pd.language_id = '" . (int)$languages_id . "' and p.suppliers_id = '" . (int)$suppliers_id . "'");
      $product = tep_db_fetch_array($product_query);
      ...
      <tr>
        <td><table border="0" cellspacing="0" cellpadding="2">
          ...
          <tr>
            ...
            <td class="main"><?php echo tep_draw_hidden_field('categories_id', $current_category_id); ?></td>
          </tr>
          <tr>
            <td colspan="2"><?php echo tep_draw_separator('pixel_trans.gif', '1', '10'); ?></td>
          </tr>
...
?>
      ...
      <tr>
        <td class="main" align="right"><?php echo tep_draw_hidden_field('products_date_added', (tep_not_null($pInfo->products_date_added) ? $pInfo->products_date_added : date('Y-m-d'))) . '<input src="images/button_preview.gif" alt="Preview" title=" Preview " type="image" border="0">' . '&nbsp;&nbsp;<a href="' . tep_href_link(FILENAME_SUPPLIER_S_CATEGORIES_PRODUCTS, 'cPath=' . $cPath . (isset($HTTP_GET_VARS['pID']) ? '&pID=' . $HTTP_GET_VARS['pID'] : '')) . '"><img src="images/button_cancel.gif" alt=" Cancel " title=" Cancel " border="0"></a>'; ?></td>
      </tr>
    </table></form>
<?php
    ...
    if (tep_not_null($HTTP_POST_VARS)) {
      ...
      $product_query = tep_db_query("select p.products_id, pd.language_id, pd.products_name, pd.products_description, pd.products_url, p.products_quantity, p.products_model, p.products_image, p.products_price, p.products_weight, p.products_date_added, p.products_last_modified, p.products_date_available, p.products_status, p.manufacturers_id, p.suppliers_id  from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd where p.products_id = pd.products_id and p.products_id = '" . (int)$HTTP_GET_VARS['pID'] . "' and p.suppliers_id = '" . (int)$suppliers_id . "'");
      $product = tep_db_fetch_array($product_query);
...
?>
      <tr>
        <td align="right"><?php echo '<a href="' . tep_href_link($back_url, $back_url_params, 'NONSSL') . '">' . tep_sup_image_button('button_back.gif', IMAGE_BACK) . '</a>'; ?></td>
      </tr>
<?php
    } else {
?>
...
/* Re-Post all POST'ed variables */
      ...
      echo '<input src="images/button_back.gif" alt="Back" title=" Back " name="edit" type="image" border="0">' . '&nbsp;&nbsp;';

      if (isset($HTTP_GET_VARS['pID'])) {
        echo '<input src="images/button_update.gif" alt="Update" title=" Update " type="image" border="0">';
      } else {
        echo '<input src="images/button_insert.gif" alt="Insert" title=" Insert " type="image" border="0">';
      }
      echo '&nbsp;&nbsp;<a href="' . tep_href_link(FILENAME_SUPPLIER_S_CATEGORIES_PRODUCTS, 'cPath=' . $cPath . (isset($HTTP_GET_VARS['pID']) ? '&pID=' . $HTTP_GET_VARS['pID'] : '')) . '"><img src="images/button_cancel.gif" alt=" Cancel " title=" Cancel " border="0"></a>';
?></td>
      </tr>
    </table></form>
<?php
...
<?php
	...
	echo tep_draw_hidden_field('osCAdminID', $osCAdminID);
...
<?php
    ...
    if (isset($HTTP_GET_VARS['search'])) {
      ...
      $products_query = tep_db_query("select p.supplier_id, p.products_id, pd.products_name, p.products_quantity, p.products_image, p.products_price, p.products_date_added, p.products_last_modified, p.products_date_available, p.products_status, p.products_model, p.suppliers_id from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd, " . TABLE_PRODUCTS_TO_CATEGORIES . " p2c where p.products_id = pd.products_id and pd.language_id = '" . (int)$languages_id . "' and p.products_id = p2c.products_id and p2c.categories_id = '" . (int)$current_category_id . "' and p.suppliers_id = '" . (int)$suppliers_id ."' order by p.Products_model");
    }
...
?>
                <td class="dataTableContent"><?php echo '<a href="' . tep_href_link(FILENAME_SUPPLIER_S_CATEGORIES_PRODUCTS, 'cPath=' . $cPath . '&pID=' . $products['products_id'] . '&action=new_product_preview&read=only') . '">' . tep_image(DIR_WS_ICONS . 'preview.gif', ICON_PREVIEW) . '</a>&nbsp;' . $products['products_name']; ?></td>
...
?></td>
                <td class="dataTableContent" align="right"><?php if (isset($pInfo) && is_object($pInfo) && ($products['products_id'] == $pInfo->products_id)) { echo tep_image(DIR_WS_IMAGES . 'icon_arrow_right.gif', ''); } else { echo '<a href="' . tep_href_link(FILENAME_SUPPLIER_S_CATEGORIES_PRODUCTS, 'cPath=' . $cPath . '&pID=' . $products['products_id']) . '">' . tep_image(DIR_WS_IMAGES . 'icon_info.gif', IMAGE_ICON_INFO) . '</a>'; } ?>&nbsp;</td>
              </tr>
<?php
    }
...
<?php
    ...
    switch ($action) {
      case 'new_category':
		...
		$contents[] = array('align' => 'center', 'text' => '<br>' . tep_sup_image_submit('button_save.gif', IMAGE_SAVE) . ' <a href="' . tep_href_link(FILENAME_SUPPLIER_S_CATEGORIES_PRODUCTS, 'cPath=' . $cPath) . '">' . tep_sup_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>');
        break;
      ...
      case 'edit_category':
        ...
        $contents[] = array('text' => '<br>' . tep_image(HTTP_CATALOG_SERVER. DIR_WS_CATALOG_IMAGES . $cInfo->categories_image, $cInfo->categories_name) . '<br>' . DIR_WS_CATALOG_IMAGES . '<br><b>' . $cInfo->categories_image . '</b>');
          $contents[] = array('text' => '<br>' . TEXT_EDIT_CATEGORIES_IMAGE . '<br>' . tep_draw_file_field('categories_image'));
        $contents[] = array('text' => '<br>' . TEXT_EDIT_SORT_ORDER . '<br>' . tep_draw_input_field('sort_order', $cInfo->sort_order, 'size="2"'));
		$contents[] = array('align' => 'center', 'text' => '<br>' . tep_sup_image_submit('button_save.gif', IMAGE_SAVE) . ' <a href="' . tep_href_link(FILENAME_SUPPLIER_S_CATEGORIES_PRODUCTS, 'cPath=' . $cPath . '&cID=' . $cInfo->categories_id) . '">' . tep_sup_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>');
        break;
      case 'delete_category':
        ...
        $contents[] = array('align' => 'center', 'text' => '<br>' . tep_sup_image_submit('button_delete.gif', IMAGE_DELETE) . ' <a href="' . tep_href_link(FILENAME_SUPPLIER_S_CATEGORIES_PRODUCTS, 'cPath=' . $cPath . '&cID=' . $cInfo->categories_id) . '">' . tep_sup_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>');
        break;
      case 'move_category':
        ...
        $contents[] = array('align' => 'center', 'text' => '<br>' . tep_sup_image_submit('button_move.gif', IMAGE_MOVE) . ' <a href="' . tep_href_link(FILENAME_SUPPLIER_S_CATEGORIES_PRODUCTS, 'cPath=' . $cPath . '&cID=' . $cInfo->categories_id) . '">' . tep_sup_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>');
        break;
      ...
      case 'delete_product':
        ...
        $contents[] = array('align' => 'center', 'text' => '<br>' . tep_sup_image_submit('button_delete.gif', IMAGE_DELETE) . ' <a href="' . tep_href_link(FILENAME_SUPPLIER_S_CATEGORIES_PRODUCTS, 'cPath=' . $cPath . '&pID=' . $pInfo->products_id) . '">' . tep_sup_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>');
        break;
      case 'move_product':
...
$contents = array('form' => tep_draw_form('products', FILENAME_SUPPLIER_S_CATEGORIES_PRODUCTS, 'action=move_product_confirm&cPath=' . $cPath) . tep_draw_hidden_field('products_id', $pInfo->products_id));
      ...
      $contents[] = array('align' => 'center', 'text' => '<br>' . tep_sup_image_submit('button_move.gif', IMAGE_MOVE) . ' <a href="' . tep_href_link(FILENAME_SUPPLIER_S_CATEGORIES_PRODUCTS, 'cPath=' . $cPath . '&pID=' . $pInfo->products_id) . '">' . tep_sup_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>');
      break;
    case 'copy_to':
      ...
      $contents[] = array('align' => 'center', 'text' => '<br>' . tep_sup_image_submit('button_copy.gif', IMAGE_COPY) . ' <a href="' . tep_href_link(FILENAME_SUPPLIER_S_CATEGORIES_PRODUCTS, 'cPath=' . $cPath . '&pID=' . $pInfo->products_id) . '">' . tep_sup_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>');
      break;
    default:
      if ($rows > 0) {
        if (isset($cInfo) && is_object($cInfo)) { // category info box contents
          $heading[] = array('text' => '<b>' . $cInfo->categories_name . '</b>');
	if($login == $cInfo->supplier_id)
	{
         $contents[] = array('align' => 'center', 'text' => '<a href="' . tep_href_link(FILENAME_SUPPLIER_S_CATEGORIES_PRODUCTS, 'cPath=' . $cPath . '&cID=' . $cInfo->categories_id . '&action=edit_category') . '"><img src="images/button_edit.gif" alt="Edit" title=" Edit " border="0"></a> <a href="' . tep_href_link(FILENAME_SUPPLIER_S_CATEGORIES_PRODUCTS, 'cPath=' . $cPath . '&cID=' . $cInfo->categories_id . '&action=delete_category') . '"><img src="images/button_delete.gif" alt="Delete" title=" Delete " border="0"></a> <a href="' . tep_href_link(FILENAME_SUPPLIER_S_CATEGORIES_PRODUCTS, 'cPath=' . $cPath . '&cID=' . $cInfo->categories_id . '&action=move_category') . '"><img src="images/button_move.gif" alt="Move" title=" Move " border="0"></a>');
	}
...
if($login== $pInfo->supplier_id)
{
          $contents[] = array('align' => 'center', 'text' => '<a href="' . tep_href_link(FILENAME_SUPPLIER_S_CATEGORIES_PRODUCTS, 'cPath=' . $cPath . '&pID=' . $pInfo->products_id . '&action=new_product') . '"><img src="images/button_edit.gif" alt="Edit" title=" Edit " border="0"></a> <a href="' . tep_href_link(FILENAME_SUPPLIER_S_CATEGORIES_PRODUCTS, 'cPath=' . $cPath . '&pID=' . $pInfo->products_id . '&action=delete_product') . '"><img src="images/button_delete.gif" alt="Delete" title=" Delete " border="0"></a> <a href="' . tep_href_link(FILENAME_SUPPLIER_S_CATEGORIES_PRODUCTS, 'cPath=' . $cPath . '&pID=' . $pInfo->products_id . '&action=move_product') . '"><img src="images/button_move.gif" alt="Move" title=" Move " border="0"></a> <a href="' . tep_href_link(FILENAME_SUPPLIER_S_CATEGORIES_PRODUCTS, 'cPath=' . $cPath . '&pID=' . $pInfo->products_id . '&action=copy_to') . '"><img src="images/button_copy_to.gif" alt="Copy To" title=" Copy To " border="0"></a>');
		 }
